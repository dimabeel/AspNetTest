...
namespace AutoLotDALCore.DataInitialization
{
    public static class MyDataInitializer
    {
        ...
        private static void ExecuteDeleteSql(AutoLotContext context,
            string tableName)
        {
            // With 2.0, must separate string interpolation if
        ...
        private static void ResetIdentity(AutoLotContext context)
        {
            ...
            {
                // With 2.0, must separate string interpolation
                // if not passing in params
                var rawSqlString = $"DBCC CHECKIDENT (\"dbo.{table}\",RESEED, -1);";
                context.Database.ExecuteSqlRaw(rawSqlString);
            }
        }
    }
